...
const Preloader = ({
  ...
  useEffect(() => {
    // Enhanced bot detection to protect SEO and PageSpeed scores
    const isBot = typeof navigator !== 'undefined' && 
      (/bot|googlebot|crawler|spider|robot|crawling|lighthouse|chrome-lighthouse|pagespeed|ptst/i.test(navigator.userAgent || '') || navigator.webdriver);
    
    // Only show once per session to avoid annoying users on reload and protect CrUX metrics
